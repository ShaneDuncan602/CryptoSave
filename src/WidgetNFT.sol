...
contract WidgetNFT is ERC721, Ownable
{
    ...
    address private cryptoSaveAddress;
    address private svgModelAddress;

    constructor() ERC721("CryptoSaveWidgetNFT", "CSW") {
    }
    ...
    function buildSvg() public view returns (bytes memory) 
    {
        ...
        parts[7] = string.concat("TBD", SvgModel.getStaticElements());
    ...
    function setCryptoSaveAddress(address _address) external onlyOwner{
        cryptoSaveAddress = _address;
    }

    function getCryptoSaveAddress() external view returns(address){
        return cryptoSaveAddress;
    }
