...
import "./CryptoSave.sol";

contract DynamicNFT is ERC721, CryptoSave {
    CryptoSave c;
    ...
    function buildSvg() public view returns (bytes memory) {
        ...
        parts[0] = string.concat(
            ...
            getIsInMoney() ? "16cb19" : "cb1616" // BG_GREEN : BG_RED
            ); 

        parts[1] = "' d='M0 0h500v500H0z'/>";

        parts[2] = string.concat(
            getIsInMoney() ? getSmileEmoji() : getPooEmoji(), 
            getIsInMoney() ? ARROW_LEFT_UP : ARROW_LEFT_DOWN
            );
        ...
        parts[5] = string.concat(c.assetHeldSymbol(), "</text>"); // Is concat cheaper than abi.encode ??
        
        parts[6] = string.concat(
            getIsInMoney() ? ARROW_RIGHT_UP : ARROW_RIGHT_DOWN, 
